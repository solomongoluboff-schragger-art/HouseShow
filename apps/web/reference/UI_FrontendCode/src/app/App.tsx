...
export default function App() {
  ...
  const handleLogin = (email: string, password: string) => {
    ...
    // TODO: Get user type from backend based on login credentials
    const mockUserType = 'artist'; // This should come from backend
    setUserType(mockUserType);
    // Set default page based on user type
    if (mockUserType === 'artist') {
      setCurrentPage('venues');
    } else if (mockUserType === 'host') {
  ...
  const handleAccountCreation = (userData: any) => {
    // Mock account creation - in production this would save to backend
    setIsAuthenticated(true);
  ...
  const handleDevNavigate = (page: string, devUserType?: 'artist' | 'host' | 'fan') => {
    if (page === 'home') {
      ...
      setIsAuthenticated(false);
      setSignupUserType(devUserType || 'artist');
      setAuthPage('signup');
    } else {
      // Authenticated pages
      if (!isAuthenticated) {
  ...
  if (!isAuthenticated) {
    ...
    if (authPage === 'login') {
      return (
        <>
          <Login onLogin={handleLogin} onBack={() => setAuthPage('home')} />
          <DeveloperToolbar onNavigate={handleDevNavigate} />
        </>
      );
    }
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4">
              <Button
                ...
                variant={currentPage === 'shows' ? 'default' : 'outline'}
                className={currentPage === 'shows' ? 'bg-primary text-primary-foreground font-semibold' : 'font-semibold'}
              >
                Upcoming Shows
              </Button>
